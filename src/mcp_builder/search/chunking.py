...
def chunks(markdown: str, count: Callable[[str], int], limit: int = 480):
    ...
    for heading, section in sections(markdown):
        ...
        for position, line in enumerate(lines):
            ...
            if marker and fence is None:
                ...
                if (current.strip() and count("passage: " + block) <= limit
                        and count("passage: " + current + block) > limit):
            ...
            suffix = "\n" + next_fence + "\n" if next_fence else ""
            if count("passage: " + current + line + suffix) <= limit:
                current += line
            else:
                if current.strip():
                    yield heading, current + ("\n" + fence + "\n" if fence else "")
                ...
                remaining = line
                while count("passage: " + current + remaining + suffix) > limit:
                    lo, hi = 0, len(remaining)
                    while lo < hi:
                        mid = (lo + hi + 1) // 2
                        if count("passage: " + current + remaining[:mid] + suffix) <= limit:
                            lo = mid
                        else:
                            hi = mid - 1
                    if lo == 0:
