...
from a revision *release* rather than from a revision *identifier*, so it does not depend on the
...
* **Specification revision support window.** The NYC meeting also discussed how long Tier 1 SDKs
  must support a given specification *revision* (as distinct from a feature within one). That
